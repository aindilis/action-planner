...
use ActionPlanner::GUI::TabManager;
...
sub init {
  ...
  $self->LoadMenus();
  $self->MyTabManager
    (ActionPlanner::GUI::TabManager->new
     ());
  $self->MyMinibuffer
  ...
  $self->MyKeyBindings
    (KMax::Util::KeyBindings->new
     (
      KeyBindingsFile => "/var/lib/myfrdcsa/codebases/minor/action-planner/keybindings-action-planner-official.txt",
      MyGUI => $self,
     ));
...
sub Exit {
  ...
  if (1) {
    ...
    if ($res eq "Exit") {
      ...
      system "(sleep 1; /var/lib/myfrdcsa/codebases/minor/action-planner/action-planner) &";
      exit(0);
    } elsif ($res eq "Reinvoke") {
      # kill it and start a new one
      my $cli = GetCommandLineForCurrentProcess();
      system "(sleep 1; cd /var/lib/myfrdcsa/codebases/minor/action-planner && $cli) &";
      exit(0);
    } elsif ($res eq "Cancel") {
      # do nothing
    }
  } else {
    if (Approve("Exit Action Planner?")) {
      exit(0);
    }
  }
}
